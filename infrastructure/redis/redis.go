	...
	redisPool = &redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			...
			if _, err = c.Do("AUTH", pswd); err != nil {
				err = fmt.Errorf("redis auth password error: %s", err.Error())
				log.Error(err)
				return nil, err
			}
			if _, err = c.Do("SELECT", db); err != nil {
				c.Close()
				err = fmt.Errorf("redis SELECT   error: %s", err.Error())
				log.Error(err)
				return nil, err
			}
			return c, nil
...
func HMHINCRBY(k string, filed interface{}, incre int, c redis.Conn) error {
	if c == nil {
		c = redisPool.Get()
		defer c.Close()
	}
...
//HMGet 哈希表get
func HMGet(k string, field interface{}, c redis.Conn) (interface{}, error) {

	if c == nil {
		c = redisPool.Get()
		defer c.Close()
	}
	result, err := c.Do("HMGet", k, field)
	if err != nil {
		return nil, err
	}
	return result, nil
...
func HMGetPojo(k string, filed interface{}, pojo DBPojo, c redis.Conn) error {
	...
	if err != nil {
		return fmt.Errorf("HMGetPojo  k:%v, filed:%v. err:%v", k, filed, err.Error())
	}
	...
	if err != nil {
		return fmt.Errorf("HMGetPojo Unmarshal dbpojoJSON:%v   --- error:%v", dbpojoJSON[0], err.Error())
	}
	return nil
}
...
//IncreKey 值累加1
func IncreKey(k string, c redis.Conn) int {
	if c == nil {
		c = redisPool.Get()
		defer c.Close()
	}
...
func DianZhan(tablename, channelStr, docid string, userid int, c redis.Conn) int {
	...
	log.Error(key, userid, result, err.Error())
