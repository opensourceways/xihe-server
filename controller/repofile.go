...
func (ctl *RepoFileController) ContainReadme(ctx *gin.Context) {
	...
	b := checkReamde(v)
	req := RepoContainReadmeRequest{}
	req.HasReadme = b
	ctx.JSON(http.StatusOK, newResponseData(req))
}
...
func checkReamde(v []platform.RepoPathItem) (b bool) {
	for i := range v {
		if v[i].Path == "README.md" {
			return true
		}
	}
	return false
